...
export default {
  ...
  methods: {
    ...
    saveToAlbum(canvas, imgData) {
      try {
        // 现代浏览器支持 navigator.clipboard 或 MediaRecorder
        if (navigator.clipboard && navigator.clipboard.write) {
          ...
          canvas.toBlob(async (blob) => {
            try {
              ...
              // 剪贴板API失败，回退到传统方法
              this.fallbackSaveToAlbum(imgData);
            }
          });
        } else {
          // 不支持剪贴板API，使用传统方法
